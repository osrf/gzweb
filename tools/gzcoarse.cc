...
struct PointCloud
{
  ...
  inline T kdtree_distance(const T *p1, const unsigned int idx_p2,
    ...
    TiXmlElement *_meshXml, const char *_meshID)
{
  ...
  const unsigned int num_results = 30;
  std::vector<unsigned int> result_index(num_results);
  ...
  for (int i = 0; i < outTriIndexCount; ++i)
  {
    ...
    {
      // Current vertex
      unsigned int currentIndex = closestIndices[k];
      gazebo::math::Vector2d
